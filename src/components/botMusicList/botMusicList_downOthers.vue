			...
			_initPullUpLoad() {
				...
				this.scroll.on('pullingUp', () => {
					...
					this.$http.get(this.LOCALAPI)
				      .then( (res) => {  
				      	this.newData.splice(this.newData.length, 0 ,{"name":"Timzt-Bigbrother"})
				      	localStorage.setItem('localMusicData',JSON.stringify(this.newData))
				      	// 出现Unexpected token o in JSON at position 1 调用localStorage.clear();
						return res
				      }).then(()=>{
				      	setTimeout(()=>{
				      		this.beforePullUp = true;
							this.isPullUpLoad = false;
							this.scroll.finishPullUp() //页面底部 每次加载完成 写一下
				      	}, 3000)
				      }).catch( (err) => {
				          console.log(err);
				      });
				...
				//上拉的过程
				this.scroll.on('scroll', (pos) => {
				  if (this.beforePullUp) {
				  	if(pos.y<-50){
					    this.bubbleY = Math.max(0, pos.y + this.pullDownInitTop)
					    this.pullDownStyle = `top:${Math.min(pos.y + this.pullDownInitTop, 10)}px`
					    console.log(pos.y+'----------------我在上拉--'+this.bubbleY)
